...
class DroneAgent:
    ...
    def select_action(self, obs0):
        ...
        if self.current_round == 0:
            self.initial_packages_num = len(obs0[PACKAGES])

        # if self.current_round > 27: print(f"{self.current_round = }")

        if (self.current_round == 29) and (len(obs0[PACKAGES]) < (0.5 * self.initial_packages_num)):
            # print(f"{self.current_round = }, {self.initial_packages_num = }, {len(obs0[PACKAGES]) = }")
            return RESET
    ...
    def update(self, obs0, action, obs1, reward):
        ...
        if action in self.learning_actions:
            ...
            old_q_value = self.q_values[obs0_as_key][action_index]
            ...
            self.q_values[obs0_as_key][action_index] = (
...
#
# class DroneAgent:
#     def __init__(self, n, m, alpha, gamma):
#         self.mode = 'train'  # do not change this!
#
#         self.alpha = alpha
#         self.gamma = gamma
...
#             self.add_obs_if_not_exists(obs1_as_key)
#
#             td_value = reward + self.gamma * np.max(
#                 self.q_values[obs1_as_key]) - old_q_value
#
#             self.q_values[obs0_as_key][action_index] = (
#                     old_q_value + self.alpha * td_value)
#
